...
export function SelectedFiles({ selectedItems }: SelectedFilesProps) {
    ...
    const groupedFiles = selectedItems.reduce((acc: { [key: string]: string[] }, path) => {
                    ...
                    <div>
                        ...
                        <div className="space-y-1 pl-6 grid grid-cols-3 gap-2">
                            {groupedFiles[''].map((fileName) => (
                                <div key={fileName} className="flex items-center gap-2 text-sm text-white/60 hover:text-white/80 transition-colors border p-3 border-white/10 rounded-lg">
